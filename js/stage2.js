...
var stage2 = function (){
	...
	var bgm = game.assets['sound/stage2.mp3'];
	bgm.play();
	...
	function moveEnemy(){
		...
		for(var i=0; i<count; i++){
			...
			if (enemy[i].y > 650){
				...
				game.pause();
				setTimeout(function(){game.replaceScene(gameOver());game.resume();}, 1000);
		...
		for(var i=0; i<count; i++){
			if (beam.intersect(enemy[i])){
				...
				if (total < 1){	// 全部倒したか調べる
					bgm.stop();
					game.pause();
					setTimeout(function(){game.replaceScene(stage3());game.resume();}, 2000);
					return;
				}
			}
		}
	...
	function drawEnemy(){
		...
		for(var y=0; y<2; y++){
			for(var x=0; x<10; x++){
				enemy[count] = new Sprite(64, 64);
				enemy[count].image = game.assets["images/monster2.png"];
				enemy[count].x = x * (64+40);	// X座標
	...
	function initEnemyLaser(){
		for(var i=0; i<maxLaser; i++){
			enemyLaser[i] = new Sprite(60, 60);
			enemyLaser[i].image = game.assets["images/mizu.png"];

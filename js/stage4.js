...
var stage4 = function (){
	...
	var bgm = game.assets['sound/stage4.mp3'];
	bgm.play();
	var beamsound = game.assets['sound/beam.mp3'];
	var burst = game.assets['sound/burst.mp3'];
	var die = game.assets['sound/die.wav'];
	...
	background.image =  game.assets['images/stage4.png'];
	background.x = 0;
	background.y = 0;
	scene.addChild(background);

    var lifeLabel = new Label("Boss Life : 15");
	lifeLabel.font = "36px Tahoma";
		...
		for(var i=0; i<count; i++){
			if (beam.intersect(enemy[i])){
				...
				if (life < 1){	// 全部倒したか調べる
					...
					bgm.stop();
					game.pause();
					setTimeout(function(){game.replaceScene(ending());game.resume();}, 1000);
					return;
	...
	function drawEnemy(){
		...
		for(var y=0; y<1; y++){
			for(var x=0; x<1; x++){
				enemy[count] = new Sprite(192, 192);
				enemy[count].image = game.assets["images/monster4.png"];
				enemy[count].x = x * (96+10);	// X座標
				enemy[count].y = y * 96 + 30;	// Y座標
	...
	function startEnemyLaser(){
		var pointer = Math.floor(Math.random() * 20);	// レーザービームを発射する敵の配列位置を求める

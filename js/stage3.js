...
var stage3 = function (){
	...
	// TODO
	// 	シーンの設定
	var bgm = game.assets['sound/stage3.mp3'];
	bgm.play();
	...
	background.image =  game.assets['images/stage3.png'];
	background.x = 0;
	background.y = 0;
...
// =============== 各種処理 ==================
		...
		for(var i=0; i<count; i++){
			if (beam.intersect(enemy[i])){
				...
				if (total < 1){	// 全部倒したか調べる
					bgm.stop();
					game.pause();
					setTimeout(function(){game.replaceScene(stage4());game.resume();}, 2000);
					return;
				}
			}
		}
	...
	function drawEnemy(){
		...
		for(var y=0; y<1; y++){
			for(var x=0; x<1; x++){
				enemy[count] = new Sprite(44, 64);
				enemy[count].image = game.assets["images/monster3.png"];
	...
	function initEnemyLaser(){
		for(var i=0; i<maxLaser; i++){
			enemyLaser[i] = new Sprite(100, 100);
			enemyLaser[i].image = game.assets["images/laser3.png"];
